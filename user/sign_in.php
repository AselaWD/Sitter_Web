...
<?php
  ...
  if(isset($_POST['login_submit'])){
    ...
    if(empty($errors)){
      ...
      $username = mysqli_real_escape_string($connection2, $_POST['user_name']);
      $pass = mysqli_real_escape_string($connection2, $_POST['user_password']);
      $hashed_password=sha1($pass);
      echo $hashed_password;
      //prepair database quary
      $query= "SELECT * FROM `usr_table` WHERE `usrnme` = '$username' AND `password` = '$hashed_password'LIMIT 1";
      ...
      if($result_set){
        ...
        if(mysqli_num_rows($result_set) == 1){
          ...
          //updating last login
          $query="UPDATE `usr_table` SET `lastlogin` = NOW()";
        ...
        <div class="login-html">
            ...
            ?>
             ...
             if(isset($_GET['logout'])){
              echo '<p id="pmsg1">You Have successfully logged out from the system</p>';
            }
            ?>
